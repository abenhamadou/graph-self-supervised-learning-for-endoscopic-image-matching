...
class PatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_key_src = torch.tensor(np.array(gt_key_src), dtype=torch.float32)
        score_src = torch.tensor(np.ones((1, gt_key_src.shape[0])), dtype=torch.float32)
        patches_next_frame = torch.tensor(np.array(patches_next_frame), dtype=torch.float32)
        gt_key_next_frame = torch.tensor(np.array(gt_key_next_frame), dtype=torch.float32)
        score_dst = torch.tensor(np.ones((1, gt_key_src.shape[0])), dtype=torch.float32)
